...
class ColorThief(object):
    ...
    def get_palette(self, color_count: int = 10, quality: int = 10) -> List[Tuple[int, int, int]]:
        ...
        cmap: CMap = MMCQ.quantize(valid_pixels, color_count)
        return cmap.palette
...
class VBox(object):
    ...
    def __init__(self, r1: int, r2: int, g1: int, g2: int, b1: int, b2: int, histo: Dict[int, int]) -> None:
        ...
        self._avg: tuple[int, int, int] | None = None
    ...
    def volume(self) -> int:
        if not self._volume:
            ...
            self._volume = (sub_r + 1) * (sub_g + 1) * (sub_b + 1)
        return self._volume
...
class PQueue(object):
    ...
    def push(self, o: VBox | Any) -> None:
        self.contents.append(o)
        self._sorted = False

    def peek(self, index: int | None = None) -> VBox:
        if not self._sorted:
            self.sort()
        if index is None:
